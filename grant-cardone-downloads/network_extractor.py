#!/usr/bin/env python3
"""
Network-based Grant Cardone Video Extractor
Extracts video URLs from browser network requests
"""
...
class NetworkExtractor:
    ...
    def run_network_extraction(self):
        """Run browser network extraction script"""
        print("🔍 Network-based Video Extraction")
        print("=" * 40)

        js_code = """
// Grant Cardone Network Video Extractor
(function() {
    ...
    // Generate navigation script to collect all videos
    function createNavigationScript() {
        return `
            // Find all course/program links and navigate to them
            const courseLinks = document.querySelectorAll('a[href*="course"], a[href*="program"], a[href*="lesson"]');
            console.log('Found ' + courseLinks.length + ' course links');

            // Collect all video data from current page
            window.extractedVideos = ${JSON.stringify(videos)};

            // Return the data
            window.extractedVideos;
        `;
    }
...
"""
        ...
        print("📜 JavaScript extraction script ready!")
        print("\n👉 INSTRUCTIONS:")
        print("1. Open Chrome and go to https://training.grantcardone.com/library")
        print("2. Open Developer Tools (Cmd+Opt+I)")
        print("3. Go to Console tab")
        print("4. Copy and paste the JavaScript above")
        print("5. Press Enter and wait 5 seconds")
        print("6. Copy the JSON output")
        print("7. Save it as 'grantcardone_videos.json' in this directory")
    ...
    def download_videos(self, videos):
        ...
        for i, video in enumerate(videos, 1):
            print(f"\n[{i}/{len(videos)}] {video.get('title', f'Video {i}')}")
    ...
    def run(self):
        """Main execution"""
        # Show the JavaScript for extraction
        self.run_network_extraction()

        # Load extracted video data if available
        try:
            with open('grantcardone_videos.json', 'r') as f:
                data = json.load(f)
                ...
                if videos:
                    ...
                    print("❌ No videos found in grantcardone_videos.json")

        except FileNotFoundError:
            print("\n⚠️  grantcardone_videos.json not found")
            print("Please run the JavaScript extraction first")
        except json.JSONDecodeError:
            print("❌ Invalid JSON in grantcardone_videos.json")
